...
export default function Bible({ route }) {
  ...
  // Estado para armazenar seleção do usuário
  const [selectedBook, setSelectedBook] = useState(null);
  const [selectedChapter, setSelectedChapter] = useState(null);
  const [selectedVerse, setSelectedVerse] = useState(null);
  const [view, setView] = useState('books');
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('Culto'); // Sempre "Culto" por padrão
  const books = BibleData?.biblia?.livros || [];

  // Inicialização com parâmetros da rota, se houver
  useEffect(() => {
